...
export class AtlasModelSource {
  ...
  async loadSchemaPackages(nsUri: string): Promise<EPackage[]> {
    ...
    // Dateiname = Paketname, damit Selbstreferenzen der Form
    // `<paketname>.ecore#//X` auflösen (siehe cascadeLoader.resourceUriFor).
    const name = content.match(/<ecore:EPackage[^>]*?\bname="([^"]+)"/)?.[1];
    const pkg = registerEcoreFromString(
      content,
      name ? `${name}.ecore` : `atlas/${encodeURIComponent(nsUri)}.ecore`,
    );
    return [pkg];
  }
}
